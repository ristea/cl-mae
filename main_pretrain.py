...
def get_args_parser():
    ...
    parser.add_argument('--lambda_kl', default=0.1, type=float)
    parser.add_argument('--lambda_diversity', default=2.0, type=float)    
...
def main(args):
    ...
    model = models_mae.__dict__[args.model](norm_pix_loss=args.norm_pix_loss, img_size=args.input_size,
                                            ...
                                            lambda_diversity=args.lambda_diversity).float()
    model.to(device)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch <  (args.epochs/2):
            curriculum_factor = 1 - (2 * epoch)/(args.epochs)
        else:
            curriculum_factor = 1 - (2 * (epoch+1))/(args.epochs)

        train_stats  = train_one_epoch(
            model, data_loader_train,
            optimizer, device, epoch, curriculum_factor, loss_scaler_mae, loss_scaler_mask, 
